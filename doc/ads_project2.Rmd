...
library(readr)
hma <- read_csv("../data/HazardMitigationAssistanceProjects(cleaned).csv")
hma_prop <- read_csv("../data/HazardMitigationAssistanceMitigatedProperties_cleaned.csv")
```
...
ggplot(merged_data_s, aes(x = long, y = lat, group = group, fill = count)) +
  geom_polygon() +
...
ggplot(merged_data_c, aes(x = long, y = lat, group = group, fill = count)) +
  geom_polygon() +
  scale_fill_gradient(low = "grey", high = "blue") +
  labs(title = "County-Level Heatmap") +
  theme_minimal()
